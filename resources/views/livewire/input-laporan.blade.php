    ...
    <div class="bg-white border border-gray-200 rounded" style="border-top: 2px solid #dc2626;">
        <div class="flex flex-wrap items-center gap-3 px-4 py-2">
            ...
            <div class="flex items-center gap-1.5">
                ...
                @else
                <input wire:model="shop" type="text" placeholder="Section"
            ...
            <div class="flex items-center gap-1">
                ...
                <input wire:model="date" type="date"
                    class="border border-gray-300 rounded px-2 py-1 text-xs w-30 focus:outline-none focus:border-red-400">
            ...
            {{-- Shift (dropdown from master data) --}}
            <div class="flex items-center gap-1.5">
            ...
            {{-- Hour (READ-ONLY — auto-computed from shift + OT + Friday logic) --}}
            <div class="flex items-center gap-1.5">
                <span class="text-xs text-gray-500">Hour</span>
                <span class="border border-gray-200 bg-gray-50 rounded px-2 py-1 text-xs w-14 font-bold font-mono text-center text-gray-700 select-none">
            ...
            {{-- Hari Pengganti (checkbox + optional day select) --}}
            <div class="flex items-center gap-1.5">
            ...
            <div class="flex items-center justify-between px-3 py-1.5 bg-gray-50 border-b border-gray-200">
                ...
                <div class="flex gap-1">
                    <button wire:click="addProductionRow"
                        class="flex items-center gap-0.5 px-2 py-0.5 text-xs bg-green-50 text-green-700 border border-green-300 rounded-sm cursor-pointer">
                        + Add
                    </button>
                    <button wire:click="removeLastProductionRow"
                        class="flex items-center gap-0.5 px-2 py-0.5 text-xs bg-red-50 text-red-600 border border-red-300 rounded-sm cursor-pointer">
                        × Del
                    </button>
                </div>
            </div>
            <table class="w-full text-xs">
                <thead>
                    <tr class="bg-gray-100 border-b border-gray-200">
                        ...
                        <th class="px-2 py-1.5 text-center font-medium text-gray-500 uppercase tracking-wide w-14">Plan H</th>
                        <th class="px-2 py-1.5 text-center font-medium text-gray-500 uppercase tracking-wide w-14">Act H</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr wire:key="prod-{{ $i }}" class="border-b border-gray-100">
                        <td class="p-px">
                            <input wire:model="productions.{{ $i }}.model" type="text"
                                class="w-full px-2 py-1.5 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50">
                        </td>
                        <td class="p-px">
                            <input wire:model="productions.{{ $i }}.plan_qty" type="text" inputmode="numeric"
                                class="w-full px-1 py-1.5 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center">
                        </td>
                        <td class="p-px">
                            <input wire:model="productions.{{ $i }}.act_qty" type="text" inputmode="numeric"
                                class="w-full px-1 py-1.5 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center">
                        </td>
                        <td class="p-px">
                            <input wire:model="productions.{{ $i }}.plan_h" type="text" inputmode="decimal"
                                class="w-full px-1 py-1.5 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center">
                        </td>
                        <td class="p-px">
                            {{-- wire:model.live → updates calcProcess immediately --}}
            ...
            <table class="w-full text-xs">
                ...
                <tbody>
                    ...
                    <tr wire:key="abs-{{ $i }}" class="border-b border-gray-100">
                        ...
                        <td class="p-px">
                            {{-- Note is a dropdown matching v2 _NOTE_TYPES --}}
                            <select wire:model="absences.{{ $i }}.note"
                                ...
                                @foreach(['Sakit','Izin','Alpa','Cuti','Sholat Jumat','Terlambat','Lainnya'] as $note)
                                <option>{{ $note }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td class="p-px">
                            {{-- wire:model.live → updates calcAbsence immediately --}}
        ...
        <div class="bg-white border border-gray-300 rounded-sm flex-shrink-0" style="width: 210px;">
            ...
            <table class="w-full text-xs">
                <tbody>
                    ...
                    {{-- BALANCE row — green if ~0, red otherwise --}}
                    <tr class="{{ $calcBalanceOk ? 'bg-green-50' : 'bg-red-50' }}">
                        <td class="py-1.5 pr-2 pl-2 font-bold text-gray-800 border-l-4 {{ $calcBalanceOk ? 'border-green-500' : 'border-red-500' }}">
                            BALANCE
                        </td>
            ...
            <table class="w-full text-xs" style="min-width: 960px;">
                <thead>
                    <tr class="bg-gray-100 border-b border-gray-200">
                        ...
                        <th class="px-2 py-1.5 text-left font-medium text-gray-500 uppercase tracking-wide w-16">Op/ST</th>
                        <th class="px-2 py-1.5 text-left font-medium text-gray-500 uppercase tracking-wide">Item</th>
                        <th class="px-2 py-1.5 text-center font-medium text-gray-500 uppercase tracking-wide w-14">QTY</th>
                        <th class="px-2 py-1.5 text-left font-medium text-gray-500 uppercase tracking-wide w-16">Satuan</th>
                        ...
                        <th class="px-2 py-1.5 text-left font-medium text-gray-500 uppercase tracking-wide w-20">Factor</th>
                        <th class="px-2 py-1.5 text-center font-medium text-gray-500 uppercase tracking-wide w-14">Hour</th>
                ...
                <tbody>
                    ...
                    <tr wire:key="inh-{{ $i }}" class="border-b border-gray-100">
                        ...
                        <td class="p-px"><input wire:model="inhouse.{{ $i }}.model"  type="text" class="w-full px-2 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50"></td>
                        <td class="p-px"><input wire:model="inhouse.{{ $i }}.op_st"  type="text" class="w-full px-2 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50"></td>
                        ...
                        <td class="p-px"><input wire:model="inhouse.{{ $i }}.qty"    type="text" inputmode="decimal" class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center"></td>
                        <td class="p-px">
                        ...
                        <td class="p-px">
                            <select wire:model="inhouse.{{ $i }}.factor"
                                class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 cursor-pointer">
                                @foreach(['Machine','Setting','Man','Tool','Material','Meeting','Quality','Try Cut','Preparation','Others'] as $fac)
                                <option>{{ $fac }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td class="p-px"><input wire:model="inhouse.{{ $i }}.hour"   type="text" inputmode="decimal" class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center"></td>
                        <td class="p-px">
                            {{-- wire:model.live → Lost drives calcQuality --}}
                    ...
                    @empty
                    <tr>
                        <td colspan="12" class="px-3 py-6 text-center text-gray-400 text-xs">
                            Belum ada data. Klik <strong>+ Add</strong> untuk menambah baris.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="bg-white border border-gray-300 rounded-sm">
        <div class="flex items-center justify-between px-3 py-1.5 bg-gray-50 border-b border-gray-200">
            <div class="flex items-baseline gap-2">
                ...
                <span class="text-xs text-gray-400 font-normal">(Tool / Model Change / Man / Machine / Material / Meeting / Quality and Others)</span>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs" style="min-width: 940px;">
                <thead>
                    <tr class="bg-gray-100 border-b border-gray-200">
                        <th class="px-2 py-1.5 text-center font-medium text-gray-500 uppercase tracking-wide w-8">No</th>
                        <th class="px-2 py-1.5 text-left font-medium text-gray-500 uppercase tracking-wide w-20">Model</th>
                ...
                <tbody>
                    ...
                    <tr wire:key="ls-{{ $i }}" class="border-b border-gray-100">
                        <td class="px-2 py-1 text-center text-gray-400">{{ $i + 1 }}</td>
                        <td class="p-px"><input wire:model="lineStops.{{ $i }}.model"   type="text" class="w-full px-2 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50"></td>
                        <td class="p-px"><input wire:model="lineStops.{{ $i }}.op_st"   type="text" class="w-full px-2 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50"></td>
                        ...
                        <td class="p-px"><input wire:model="lineStops.{{ $i }}.start"   type="time" class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center"></td>
                        <td class="p-px"><input wire:model="lineStops.{{ $i }}.end"     type="time" class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center"></td>
                        <td class="p-px"><input wire:model="lineStops.{{ $i }}.stop"    type="text" inputmode="decimal" class="w-full px-1 py-1 text-xs border-0 bg-transparent focus:outline-none focus:bg-gray-50 text-center text-gray-400"></td>
                        <td class="p-px">
                            {{-- wire:model.live → Lost drives calcLineStop --}}
                    ...
                    @empty
                    <tr>
                        <td colspan="11" class="px-3 py-6 text-center text-gray-400 text-xs">
                            Belum ada data. Klik <strong>+ Add</strong> untuk menambah baris.
                        </td>
                    </tr>
                    @endforelse
